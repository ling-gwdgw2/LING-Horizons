...
import me.ling.horizons.client.TimingStatistics;
...
import me.ling.horizons.client.core.gl.GlTexture;
import me.ling.horizons.client.core.model.ModelBakerySubsystem;
import me.ling.horizons.client.core.model.ModelStore;
import me.ling.horizons.client.core.rendering.ChunkBoundRenderer;
import me.ling.horizons.client.core.rendering.RenderDistanceTracker;
import me.ling.horizons.client.core.rendering.Viewport;
import me.ling.horizons.client.core.rendering.ViewportSelector;
import me.ling.horizons.client.core.rendering.building.RenderGenerationService;
...
import me.ling.horizons.client.core.rendering.section.IUsesMeshlets;
import me.ling.horizons.client.core.rendering.section.backend.AbstractSectionRenderer;
import me.ling.horizons.client.core.rendering.section.backend.mdic.MDICSectionRenderer;
import me.ling.horizons.client.core.rendering.section.geometry.BasicSectionGeometryData;
import me.ling.horizons.client.core.rendering.section.geometry.IGeometryData;
import me.ling.horizons.client.core.rendering.util.DownloadStream;
import me.ling.horizons.client.core.rendering.util.PrintfDebugUtil;
import me.ling.horizons.client.core.rendering.util.UploadStream;
import me.ling.horizons.client.core.util.GPUTiming;
import me.ling.horizons.client.core.util.IrisUtil;
import me.ling.horizons.common.Logger;
import me.ling.horizons.common.thread.ServiceManager;
...
public class LingRenderSystem {
    ...
    public LingRenderSystem(WorldEngine world, ServiceManager sm) {
        ...
        try {
...

            {
                ...
                this.setRenderDistance(LingConfig.CONFIG.sectionRenderDistance);
            }
    ...
    public Viewport<?> setupViewport(ChunkRenderMatrices matrices, double cameraX, double cameraY, double cameraZ) {
        ...
        if (viewport == null || Minecraft.getInstance().level == null || Minecraft.getInstance().player == null) {
            return null;
        }

        //Do some very cheeky stuff for MiB
        if (LingCommon.IS_MINE_IN_ABYSS) {
        ...
        if (LingClient.getOcclusionDebugState()==0) {
            viewport.frameId++;
        }
    ...
    public void renderOpaque(Viewport<?> viewport) {
        ...
        // MC 1.21.1 NeoForge: Fog is handled by LingClientEvents.onRenderFog()
        ...
        if ((!LingClient.disableSodiumChunkRender())&&!false) {
        ...
        //As much dynamic runtime stuff here
        {
            ...
            while (this.renderDistanceTracker.setCenterAndProcess(viewport.cameraX, viewport.cameraZ) && LingClient.isFrexActive());//While FF is active, run until everything is processed
            TimingStatistics.H.start();
            //Done here as is allows less gl state resetup
            do { this.modelService.tick(900_000); } while (LingClient.isFrexActive() && !this.modelService.areQueuesEmpty());
    ...
    private void autoBalanceSubDivSize() {
        ...
        if (Minecraft.getInstance().getFps() < MIN_FPS) {
            LingConfig.CONFIG.subDivisionSize = Math.min(LingConfig.CONFIG.subDivisionSize + INCREASE_PER_SECOND / Math.max(1f, Minecraft.getInstance().getFps()), 256);
        }

        if (MAX_FPS < Minecraft.getInstance().getFps() && canDecreaseSize) {
            LingConfig.CONFIG.subDivisionSize = Math.max(LingConfig.CONFIG.subDivisionSize - DECREASE_PER_SECOND / Math.max(1f, Minecraft.getInstance().getFps()), 28);
        }
    }
    ...
    private static Matrix4f computeProjectionMat(Matrix4fc base) {
        ...
        nearVoxy = LingClient.disableSodiumChunkRender()?0.1f:nearVoxy;
    ...
    private static long getGeometryBufferSize() {
        if (LingConfig.CONFIG.geometryBufferSizeMB > 0) {
            return LingConfig.CONFIG.geometryBufferSizeMB * 1024L * 1024L;
        }
