package me.ling.horizons.client.core.rendering.section.backend.mdic;


import me.ling.horizons.client.RenderStatistics;
import me.ling.horizons.client.LingClient;
import me.ling.horizons.client.core.AbstractRenderPipeline;
...
import me.ling.horizons.client.core.gl.shader.ShaderLoader;
...
import me.ling.horizons.client.core.rendering.util.LightMapHelper;
...
public class MDICSectionRenderer extends AbstractSectionRenderer<MDICViewport, BasicSectionGeometryData> {
    ...
    private final Shader prepShader = Shader.make()
            .define("CMDGEN_WORKGROUP_SIZE", CMDGEN_WORKGROUP_SIZE)
            .add(ShaderType.COMPUTE, "ling_horizons:lod/gl46/prep.comp")
            .compile();

    private final Shader cullShader = Shader.make()
            .add(ShaderType.VERTEX, "ling_horizons:lod/gl46/cull/raster.vert")
            .add(ShaderType.FRAGMENT, "ling_horizons:lod/gl46/cull/raster.frag")
            .compile();

    private final Shader prefixSumShader = Shader.make()
            //Use subgroup prefix sum if possible otherwise use dodgy... slow prefix sum
            .add(ShaderType.COMPUTE, Capabilities.INSTANCE.subgroup?"ling_horizons:util/prefixsum/inital3.comp":"ling_horizons:util/prefixsum/simple.comp")
            .define("IO_BUFFER", 0)
            .compile();
    ...
    private final Shader translucentGenShader = Shader.make()
            ...
            .defineIf("USE_SUBGROUPS", Capabilities.INSTANCE.subgroup)
            .add(ShaderType.COMPUTE, "ling_horizons:lod/gl46/buildtranslucents.comp")
            .compile();
    ...
    public MDICSectionRenderer(AbstractRenderPipeline pipeline, ModelStore modelStore, BasicSectionGeometryData geometryData) {
        ...
        String vertex = ShaderLoader.parse("ling_horizons:lod/gl46/quads3.vert");
        ...
        String frag = ShaderLoader.parse("ling_horizons:lod/gl46/quads.frag");
        ...
        this.translucentTerrainShader = tryCompilePatchedOrNormal(builder.define("TRANSLUCENT").defineIf("ENABLE_WATER_SSR", LingConfig.CONFIG.enableWaterSSR), translucentFrag, frag);
    }
    ...
    private void uploadUniformBuffer(MDICViewport viewport) {
        ...
        int earthCurveRatio = LingConfig.CONFIG.earthCurveRatio;
    ...
    private void renderTerrain(MDICViewport viewport, long indirectOffset, long drawCountOffset, int maxDrawCount) {
        ...
        if (LingClient.getOcclusionDebugState()==3) {
