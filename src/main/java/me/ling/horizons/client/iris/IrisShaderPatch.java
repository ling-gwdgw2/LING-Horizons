...
public class IrisShaderPatch {
    ...
    public static boolean isDistantShaderShadowsEnabled() {
        return me.ling.horizons.client.config.LingConfig.CONFIG.enableDistantShaderShadows;
    }
    ...
    public static IrisShaderPatch makePatch(ShaderPack ipack, AbsolutePackPath directory, Function<AbsolutePackPath, String> sourceProvider) {
        String patchJson = sourceProvider.apply(directory.resolve("ling_horizons.json"));
        if (patchJson == null) {
            // External shaderpack compatibility fallback (Photon, Complementary, Bliss)
            patchJson = sourceProvider.apply(directory.resolve("voxy.json"));
        }
        if (patchJson == null) {
            return null;
        }
        ...
        try {
            {
                StringBuilder builder = new StringBuilder(patchJson.length());
                //Rebuild the patch, replacing commented out " with \"
                for (var line : patchJson.split("\n")) {
                ...
                patchJson = builder.toString();
            }
            patchData = GSON.fromJson(patchJson, PatchGson.class);
            if (patchData == null) {
                throw new IllegalStateException("LING Horizons patch json returned null, this is most likely due to malformed json file");
            }

            {//Inject data from the auxilery files if they are present
                var opaque = sourceProvider.apply(directory.resolve("ling_horizons_opaque.glsl"));
                if (opaque == null) {
                ...
                var translucent = sourceProvider.apply(directory.resolve("ling_horizons_translucent.glsl"));
                if (translucent == null) {
